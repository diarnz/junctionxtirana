...
async def _run_copilot(
    ...
    """
    General admin copilot.
    Uses structured backend data where possible and falls back to an LLM polish pass.
    """
...
async def _run_room_designer(
    ...
    if not room_dims:
        raise HTTPException(status_code=400, detail="No 3D dimensions available for this venue.")

    spec = _parse_layout_prompt(user_message, venue.name, room_dims)
    tool_calls.append({"tool": "parse_design_prompt", "args": {"prompt": user_message}, "result": spec})

    start_dt = context.get("event_date_start")
    end_dt = context.get("event_date_end")
    availability = await _available_models_for_window(start_dt, end_dt, db)
    tool_calls.append({"tool": "check_furniture_inventory", "args": {"start": start_dt, "end": end_dt}, "result": availability})

    layout_result = _generate_layout_from_spec(venue.three_d_room_id, room_dims, spec, availability)
    items = layout_result["items"]
    limitations = layout_result["limitations"]
    tool_calls.append({"tool": "generate_room_layout", "args": {"spec": spec}, "result": {"item_count": len(items), "limitations": limitations}})
    ...
    tool_calls.append({"tool": "push_layout_to_3d", "args": {"roomId": venue.three_d_room_id}, "result": {"item_count": len(items), "layout_id": str(layout.id) if layout else None}})

    base_response = (
        f"I configured the {venue.name} layout as a {spec['event_type']} setup with {layout_result['placed_seats']} seat positions "
        f"and {len(items)} total placed items."
    )
    if limitations:
        base_response += " Limitations: " + "; ".join(limitations)

    response = await _maybe_llm_rewrite(
        system_prompt="You are a 3D room design assistant. Explain what layout you just applied in a concise, helpful way.",
        user_prompt=f"Layout spec: {spec}\nApplied item count: {len(items)}\nLimitations: {limitations}",
...
async def _check_assets_by_name(required_assets: list[dict[str, Any]], requested_date: date, start_time: time, end_time: time, db: AsyncSession) -> list[dict[str, Any]]:
    start_dt = combine_datetime(requested_date, start_time)
    end_dt = combine_datetime(requested_date, end_time)
...
def _estimate_quotation(venue: Venue | None, duration_hours: float, asset_results: list[dict[str, Any]], event_type: str) -> dict[str, Any]:
    subtotal = Decimal("0")
...
async def _available_models_for_window(start_dt: datetime | str | None, end_dt: datetime | str | None, db: AsyncSession) -> dict[str, int]:
...
def _generate_layout_from_spec(room_id: str, room_dims: dict[str, float], spec: dict[str, Any], availability: dict[str, int]) -> dict[str, Any]:
    """
    Generates items in the exact shape consumed by the existing Three.js furnishing engine:
      {modelKey, x, y?, z, rotY, type, surfaceY?, wallAxis?, wallCoord?, isPositiveWall?, mountY?, stackOn?, lxf?, lzf?}
    """
...
def _layout_conference(room_dims: dict[str, float], spec: dict[str, Any], availability: dict[str, int]) -> dict[str, Any]:
    ...
    max_seats = cols * rows
    placed = min(attendee_target, max_seats)
...
def _layout_workshop(room_dims: dict[str, float], spec: dict[str, Any], availability: dict[str, int]) -> dict[str, Any]:
    ...
    chair_key = "simple_chair"
    tables_available = availability.get(table_key, 0)
    chairs_available = availability.get(chair_key, spec["attendee_count"])
    attendee_target = min(spec["attendee_count"], chairs_available)
    cluster_size = 4
    ...
    for row, z in enumerate(z_positions):
        for col, x in enumerate(x_positions):
            ...
            items.append({"modelKey": table_key, "x": x, "z": z, "rotY": 0.0, "type": "floor"})
            for seat_idx, (dx, dz, rot) in enumerate([(0, 0.62, math.pi), (0, -0.62, 0.0), (0.72, 0, -math.pi / 2), (-0.72, 0, math.pi / 2)]):
                if placed_chairs >= attendee_target:
                    break
                items.append({"modelKey": chair_key, "x": x + dx, "z": z + dz, "rotY": rot, "type": "floor"})
                placed_chairs += 1
...
def _layout_hackathon(room_dims: dict[str, float], spec: dict[str, Any], availability: dict[str, int]) -> dict[str, Any]:
    ...
    for z in z_positions:
        for x in x_positions:
            ...
            chair_positions = [(0, 0.62, math.pi), (0, -0.62, 0.0), (0.82, 0, -math.pi / 2), (-0.82, 0, math.pi / 2)]
            for dx, dz, rot in chair_positions:
